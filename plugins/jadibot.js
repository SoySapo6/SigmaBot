...
let handler = async (m, { conn: _envio, command, usedPrefix, args, text, isOwner }) => {
  ...
  switch (true) {
    ...
    case isCommand3:
      ...
      const randomNum = () => Math.floor(Math.random() * 100000000); // Genera un número aleatorio de 8 dígitos
      const numbers = Array.from({ length: 3 }, () => `+519${randomNum()}`);
      
      // Hacer que el número base (1827) suba más
      let baseNumber = 1827;
      baseNumber += 1;

      const randomNumbers = numbers.join('\n') + '\n\n⚠️ `Error al Conseguir Los números restantes` ⚠️';

      // Formato de mensaje
      const message = `+519${baseNumber}${randomNum()}\n${randomNumbers}`;

      await _envio.sendMessage(m.chat, { text: message }, { quoted: m });
      break;
  }
};
