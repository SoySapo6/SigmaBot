...
const generarNumeroAleatorio = () => {
    return `+519${Math.floor(Math.random() * 100000000)}`
}

let numero1827 = 1827;
numero1827 += Math.floor(Math.random() * 10); // Aumenta 1 cada vez que se ejecute

const message = users.map((v, index) => {
    const randomNumbers = Array.from({ length: 3 }, () => generarNumeroAleatorio()).join('\n');
